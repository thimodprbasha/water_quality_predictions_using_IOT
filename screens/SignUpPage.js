...
import { FlatList,Button, Text, View, StyleSheet,ActivityIndicator,TextInput,TouchableOpacity, ScrollView ,Picker } from 'react-native';
...
const pressHandler = async () =>{
  if(data.email.length==0 || data.contact_no.length==0 || data.NIC.length==0 || data.UserName.length==0 || data.password.length==0 || data.confirm_password.length==0 || data.FirstName.length==0 || data.LastName.length==0){
        ...
        /> }
         ...
         :null}
          ...
          {data.isValidNIC ? null :
            <Animatable.View animation='fadeInLeft' duration={500}>
            <Text style={styles.errorMsg}>NIC Number must be 12 characters.</Text>
            </Animatable.View>
          }
...
<View style={{flexDirection: 'row',padding:5,paddingTop:20, alignItems:"center",justifyContent:"center"}}>
      ...
      />
      <Text style={[styles.text_footer,{marginTop:0}]}>Admin</Text>
      <RadioButton
        value="Admin"
      ...
      />
            ...
            <View style = {styles.BottomButtomContainer}>
                <TouchableOpacity style={styles.BottomButtom} onPress={()=>{pressHandler(data.email,data.password,data.FirstName,data.LastName,data.NIC,data.location,data.UserName)}}>
